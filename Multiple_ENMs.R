                          ...
                          cross_validation)
{
  ...
  for (j in 1:cross_validation)
  {
    ...
    ## Predicting
    bioclim_c <-  predict(object = bioclim_model, x = current)
    writeRaster(bioclim_c, filename = paste0(Pout, j, "_bioclim_c.grd"), format = "raster")
    
    ## Evaluating models
    thr <- quantile(extract(bioclim_c, occur[,1:2]), 0.05) 
    ...
    ## Predicting
    gower_c <- predict(object = gower_model, x = current)
    writeRaster(gower_c, filename = paste0(Pout, j, "_gower_c.grd"), format = "raster")
    
    ## Evaluating models
    thr <- quantile(extract(gower_c, occur[,1:2]), 0.05) 
    TPR <- evaluate(p = testing[testing[, "pb"] == 1, -1], a = testing[testing[, "pb"] == 0, -1], 
    ...
    ## Predicting
    maxent_c <- predict(object = maxent_model, x = current)
    writeRaster(maxent_c, filename = paste0(Pout, j, "_maxent_c.grd"), format = "raster")
    ...
    ## Predicting
    SVM_c <- predict(model = SVM_model, object = current)
    writeRaster(SVM_c, filename = paste0(Pout, j, "_SVM_c.grd"), format = "raster")
    
    ## Evaluating models
    thr <- quantile(extract(SVM_c, occur[,1:2]), 0.05) 
    TPR <- evaluate(p = testing[testing[, "pb"] == 1, -1], a = testing[testing[, "pb"] == 0, -1], 
    ...
    for (i in AOGCMs)
    {
      ...
      ### Predicting
      #..........
      bioclim_rcp26 <- predict(object = bioclim_model, x = rcp26)
      writeRaster(bioclim_rcp26, filenamename = paste0(Pout, j, "_bioclim_rcp26.grd"), format = "raster")
      rm(bioclim_rcp26)
      
      bioclim_rcp45 <- predict(object = bioclim_model, x = rcp45)
      writeRaster(bioclim_rcp45, filename = paste0(Pout, j, "_bioclim_rcp45.grd"), format = "raster")
      rm(bioclim_rcp45)
      
      bioclim_rcp60 <- predict(object = bioclim_model, x = rcp60)
      writeRaster(bioclim_rcp60, filename = paste0(Pout, j, "_bioclim_rcp60.grd"), format = "raster")
      rm(bioclim_rcp60)
      
      bioclim_rcp85 <- predict(object = bioclim_model, x = rcp85)
      writeRaster(bioclim_rcp85, filename = paste0(Pout, j, "_bioclim_rcp85.grd"), format = "raster")
      rm(bioclim_rcp85)
      
      #..........
      gower_rcp26   <- predict(object = gower_model, x = rcp26)
      writeRaster(gower_rcp26,   filename = paste0(Pout, j, "_gower_rcp26.grd"), format = "raster")
      rm(gower_rcp26)
      
      gower_rcp45   <- predict(object = gower_model, x = rcp45)
      writeRaster(gower_rcp45,   filename = paste0(Pout, j, "_gower_rcp45.grd"), format = "raster")
      rm(gower_rcp45)
      
      gower_rcp60   <- predict(object = gower_model, x = rcp60)
      writeRaster(gower_rcp60,   filename = paste0(Pout, j, "_gower_rcp60.grd"), format = "raster")
      rm(gower_rcp60)
      
      gower_rcp85   <- predict(object = gower_model, x = rcp85)
      writeRaster(gower_rcp85,   filename = paste0(Pout, j, "_gower_rcp85.grd"), format = "raster")
      rm(gower_rcp85)
      
      #..........
      maxent_rcp26  <- predict(object = maxent_model, x = rcp26)
      writeRaster(maxent_rcp26,  filename = paste0(Pout, j, "_maxent_rcp26.grd"), format = "raster")
      rm(maxent_rcp26)
      
      maxent_rcp45  <- predict(object = maxent_model, x = rcp45)
      writeRaster(maxent_rcp45,  filename = paste0(Pout, j, "_maxent_rcp45.grd"), format = "raster")
      rm(maxent_rcp45)
      
      maxent_rcp60  <- predict(object = maxent_model, x = rcp60)
      writeRaster(maxent_rcp60,  filename = paste0(Pout, j, "_maxent_rcp60.grd"), format = "raster")
      rm(maxent_rcp60)
      
      maxent_rcp85  <- predict(object = maxent_model, x = rcp85)
      writeRaster(maxent_rcp85,  filename = paste0(Pout, j, "_maxent_rcp85.grd"), format = "raster")
      rm(maxent_rcp85)
      
      #...........
      SVM_rcp26     <- predict(model = SVM_model, object = rcp26)
      writeRaster(SVM_rcp26,     filename = paste0(Pout, j, "_SVM_rcp26.grd"), format = "raster")
      rm(SVM_rcp26)
      
      SVM_rcp45     <- predict(model = SVM_model, object = rcp45)
      writeRaster(SVM_rcp45,     filename = paste0(Pout, j, "_SVM_rcp45.grd"), format = "raster")
      rm(SVM_rcp45)
      
      SVM_rcp60     <- predict(model = SVM_model, object = rcp60)
      writeRaster(SVM_rcp60,     filename = paste0(Pout, j, "_SVM_rcp60.grd"), format = "raster")
      rm(SVM_rcp60)
      
      SVM_rcp85     <- predict(model = SVM_model, object = rcp85)
      writeRaster(SVM_rcp85,     filename = paste0(Pout, j, "_SVM_rcp85.grd"), format = "raster")
      rm(SVM_rcp85)
